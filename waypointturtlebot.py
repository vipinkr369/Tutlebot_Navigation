...
class RobotCommand(Node):
    def __init__(self, robot_name: str, command_type: str, coordinates: dict, node_id: int = 0):
        # Unique node name for each instance/thread
        super().__init__(f'robot_command_node_{robot_name or "tb3"}_{node_id}')
        self.robot_name = robot_name
        self.command_type = command_type.lower()
        self.coordinates = coordinates

        # Handle prefix gracefully
    ...
    def send_initial_pose(self):
        ...
        self.get_logger().info(f"✅ Initial pose published for {self.robot_name or 'turtlebot3'}: {self.coordinates}")

    # ------------------------------------------------------------------
    def send_goal_pose(self):
        if not self.client.wait_for_server(timeout_sec=5.0):
            self.get_logger().error(f"❌ Action server not available: /{self.robot_name or ''}/navigate_to_pose")
            return
...
if __name__ == "__main__":
    ...
    t1 = Thread(target=run_robot_command, args=('', 'init', coords_init, 1))
    t1.start()
